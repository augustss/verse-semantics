...
coreToTrs (CExists [] e) = coreToTrs e
coreToTrs (CExists (i:is) e) = T.Exi $ T.Bind (coreToTrsI i) (coreToTrs $ CExists is e)
...
trsToCore ee@T.Exi{} = flat [] ee
  where flat vs (T.Exi (T.Bind x e)) = flat (vs ++ [x]) e
        flat vs e = CExists (map trsToCoreI vs) (trsToCore e)
