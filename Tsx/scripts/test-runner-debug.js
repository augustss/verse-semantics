...
const { parseExpression, parseProgram, lex, PrettyPrinter } = require('../dist');
...
function runTests(filePaths, options = {}) {
  ...
  if (options.reconstruct && totalReconstructed > 0) {
...
function runSingleTest(testCode, expectError, expectTopLevel, lineStart, fileName, options = {}, testNumber = null) {
  ...
  try {
    ...
    if (expectError) {
      ...
      if (options.reconstruct && !expectError) {
        try {
          const tokenStream = lex(testCode);
          const printer = new PrettyPrinter(tokenStream);
          reconstructed = printer.print(ast);
...
console.log('\nDEBUG: totalReconstructed =', totalReconstructed);
console.log('DEBUG: totalReconstructionMatches =', totalReconstructionMatches);
console.log('DEBUG: options.reconstruct =', options.reconstruct);
