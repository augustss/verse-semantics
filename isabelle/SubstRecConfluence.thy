...
lemma renumber_subVC:
  "renumber f (subVC vc1 vc2 e) = subVC (renumber f vc1) (renumber f vc2) (renumber (liftedN 1 0 f) e)"
  sorry

lemma renumber_appVC[simp]: "renumber f (appVC vc e) = appVC (renumber f vc) (renumber (liftedN 1 0 f) e)"
  sorry
...
lemma substVC_dead:
  "\<not> occurs n vc \<Longrightarrow> subst n v vc = vc"
  sorry
...
lemma renumber_renumber: "renumber f (renumber g x) = renumber (f o g) x"
  unfolding renumber_def
  by (auto simp add: renumber_def bind_bind)

lemma [simp]: "n - Suc 0 = 0 \<longleftrightarrow> n = 0 \<or> n = 1" by auto

lemma lifted_simps[simp]: "0 < k \<Longrightarrow> liftedN k n f n = n"
  unfolding liftedN_def by auto
...
proof(rule joinI; elim rule_SubstRec.cases; clarify; goal_cases)
  ...
  next
    ...
    have inj: "renumber (id(Suc (Suc n):= 0)) (\<up> (\<up> vc1)) \<noteq>
              renumber (id(Suc (Suc n):= 0)) (\<up> (\<up> vc2))" sorry
    ...
    show ?thesis
      ...
      apply (rule congruentE[OF congruent_cc, of _ _ _  "[CDef]", simplified appEC_def, simplified])
       ...
       apply (rule cc_rootI)
       apply (subst vc1_sub)
       apply (simp only: renumber_appVC) 
       apply (rule rule_SubstRec[OF _ refl refl])
      using  `occurs (n+1) e2` apply (simp add: occurs_renumber renumber_renumber liftedN_def bumpAt_def)
      apply fastforce
      ...
      apply (rule congruentE[OF congruent_cc, of _ _ _  "[CDef]", simplified appEC_def, simplified])
       ...
       apply (simp only: renumber_appVC) 
       apply (rule rule_SubstRec[OF _ refl refl])
      using  `occurs (n+1) e1` apply (simp add: occurs_renumber renumber_renumber liftedN_def bumpAt_def)
      apply fastforce
      (* Now we have to show that the result is the same *)
      apply (simp add: subVC_swap[OF False] renumber_renumber  liftedN_def)
      apply (rule arg_cong2[where f = "\<lambda> e1 e2. appVC (subVC vc2 vc1 e1) e2" for vc1 vc2])
       apply (rule arg_cong2[where f = VLet, OF _ refl])
       apply (rule arg_cong2[where f = appVC, OF refl])
       apply (rule arg_cong2[where f = VLet])
         apply (rule arg_cong2[where f = appVC])
          apply (subst subVC_eq[OF False[symmetric] "1"(3)[symmetric]])
          apply (rule arg_cong[where f = "\<lambda>g. renumber g e" for e], rule)
          apply (fastforce simp add: bumpAt_def)
        apply (rule arg_cong[where f = "\<lambda>g. renumber g e" for e], rule)
        apply (case_tac x) apply (fastforce simp add: liftedN_def)
        apply (fastforce simp add: bumpAt_def liftedN_def)
        apply (rule arg_cong[where f = "\<lambda>g. renumber g e" for e], rule)
        apply (case_tac x) apply (fastforce simp add: liftedN_def bumpAt_def)
        apply (fastforce simp add: bumpAt_def liftedN_def)
      apply (rule arg_cong2[where f = VLet, OF _ refl])
       apply (simp add: renumber_subVC add: subVC_swap[OF inj, unfolded renumber_renumber])
       apply (rule arg_cong2[where f = "\<lambda> e1 e2. appVC (subVC vc2 vc1 e1) e2" for vc1 vc2])
       apply (rule arg_cong2[where f = VLet])
        apply (rule arg_cong2[where f = appVC])
         apply (subst (2) subVC_eq[OF False "1"(3), symmetric])
         apply (simp add: renumber_subVC)
         apply (rule arg_cong3[where f = subVC])
           apply (fastforce simp add: bumpAt_def liftedN_def renumber_renumber intro!: arg_cong[where f = "\<lambda>g. renumber g e" for e])
          apply (fastforce simp add: bumpAt_def liftedN_def renumber_renumber intro!: arg_cong[where f = "\<lambda>g. renumber g e" for e])
         apply (fastforce simp add: bumpAt_def liftedN_def renumber_renumber intro!: arg_cong[where f = "\<lambda>g. renumber g e" for e])
        apply (simp add: renumber_renumber)
        apply (rule arg_cong[where f = "\<lambda>g. renumber g e" for e], rule)
        apply (case_tac x) apply (fastforce simp add: liftedN_def)
        apply (fastforce simp add: bumpAt_def liftedN_def)
       apply (simp add: renumber_renumber)
