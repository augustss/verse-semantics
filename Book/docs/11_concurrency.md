...
Additionally, these expressions inherit the effects of all expressions they contain. If any subexpression has `<transacts>`, `<reads>`, `<writes>`, or other effects, the containing function must declare those effects as well:

```verse
TransactionalOp()<transacts><suspends>:void =
    Sleep(1.0)
    # Modifies mutable state

# Must declare transacts because TransactionalOp has it
ConcurrentTransactional()<transacts><suspends>:void =
    sync:
        TransactionalOp()  # Requires transacts effect
        TransactionalOp()
```

This effect propagation ensures that the type system accurately tracks what side effects might occur, even in complex concurrent scenarios.
...
<!--verse
using{/Verse.org/VerseCLR}
BackgroundWork()<suspends>:void={Sleep(1.0)}
F():void={
-->
```verse
...
# Safe to call on completed tasks
# (has no effect)
```
<!--verse
}
...
<!--verse
using{/Verse.org/VerseCLR}
BackgroundWork()<suspends>:int=42
F()<suspends>:void={
-->
```verse
...
If you await a task that gets canceled, the `Await()` call itself fails, allowing you to handle cancellation:

```verse
RiskyTask:task(int) = spawn{MightBeCanceled()}

# In another context, task gets canceled:
# RiskyTask.Cancel()

# Await fails if task was canceled
if (Result := RiskyTask.Await()):
...
**Note**: `Completed[]` and `Uninterrupted[]` are equivalent, as are `Canceled[]` and `Interrupted[]`. The multiple names provide semantic clarity depending on whether you're thinking about "completion" or "interruption."
...
**Event operations:**
```verse
Value := MyEvent.Await()  # Suspends until signal, checks cancellation
```
...
2. **Runs in reverse order**: Multiple defer blocks execute in last-in-first-out order (most recent defer runs first)
...
<!--verse
var Changes:[]int = array{}
RecordChange(:int):void = {set Changes = Changes + array{0}}
CommitChanges():void={}
RollbackChanges():void={}
PerformWork()<suspends><decides>:void={}
-->
```verse
AtomicOperation()<suspends>:void =
...
```verse
TestCancellation()<suspends>:void =
    WorkTask:task(void) = spawn{CancellableWork()}

    Sleep(1.0)
    WorkTask.Cancel()  # Triggers defer execution in CancellableWork
```
...
WaitUntil(TargetTime:float)<suspends>:void =
    loop:
        if (GetSecondsSinceEpoch() >= TargetTime):
            return
...
<!--verse
using{/Verse.org/Concurrency}
game_phase:=enum{Lobby,Playing,Ended}
UIUpdate(:game_phase):void={}
AIUpdate(:game_phase):void={}
AudioUpdate(:game_phase):void={}
-->
```verse
