...
physics := module:
    ...
    sphere_collider := class(CollidableShape):
        GetBounds():bounding_box = CalculateSphereBounds()
```
...
gameplay := module:
    ...
    Item<SharedGameplayScope> := class:
        ID<public>:int
        Properties<public>:map(string, string)

    # Factory function available to both systems
    CreateItem<SharedGameplayScope>(ID:int):Item = Item{ID, map{}}
...
state_manager := module:
    ...
    var<SharedScope> GameState<public>:game_state = InitialState()
...
@attribscope_class
gameplay_element := class(attribute):
    Category:string
    Priority:int
...
**Important constraints on accessors:**
...
# ERROR 3651: Cannot define inside function
BadFunction():void =
