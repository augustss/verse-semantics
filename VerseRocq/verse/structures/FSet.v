...
Definition nonempty_mem {A} (V : fset A) : Prop :=
...
Lemma mem_union_nonempty {D1 D2 : fset A}: 
  nonempty_mem D1 ->
  nonempty_mem D2 ->
  nonempty_mem (union_fset D1 D2).
Proof. intros.
 unfold nonempty_mem in *. 
Admitted.

Lemma singleton_nonempty (x : A) :
 nonempty_mem (singleton_fset x).
Admitted.
...
#[export] Hint Resolve singleton_mem mem_singleton singleton_nonempty
  nonempty_mem_nonempty nonempty_sub_nonempty_mem nonempty_nonempty_mem nonempty_mem_nonempty : core. 
#[export] Hint Resolve In_mem_Included Included_mem_In mem_union_Included mem_union_nonempty : core.
