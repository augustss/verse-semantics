...
-- We need Eq/Ord so we can make sets.
-- It's a little involved.
deriving instance (forall a . (Eq   a) => Eq   (m a)) =>
                  Eq   (Val m)
deriving instance (forall a . (Eq   a) => Eq   (m a),
                   forall a . (Ord  a) => Ord  (m a)) =>
                  Ord  (Val m)
deriving instance (forall a . (Show a) => Show (m a)) =>
                  Show (Val m)
...
class (forall a . (Ord a) => Ord (m a)) => Comp m where
  empty    :: m(a)
  inj      :: Set(a) → m(a)
  (++)     :: m(a) → m(a) → m(a)
  (⎧*⎫)    :: Ord a => m(a) → m(a) → m(a)
