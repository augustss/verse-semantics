...
invariant t = check S.empty t
 where
  ...
  check forb (IF x a yes no) =
       not (x `S.member` forb)
    && (case a of Var y -> not (y `S.member` forb); _ -> True)
    && Var x > a
    && (a,x) <. yes
    && (a,x) <. no
...
mkIF x a yes no = mkIFxa (simp x a yes) no
 where
   mkIFxa yes@(~(IF y b yes1 no1)) no@(~(IF z c yes2 no2))
     | yes==no      = yes
     | (a,x) =? no  = mkIFxa yes no2
     -- TODO optimization: choose the smallest one of the following two cases
     | (a,x) >? yes = mkIF y b (mkIFxa yes1 no) (mkIFxa no1 no)
     | (a,x) >? no  = mkIF z c (mkIFxa yes yes2) (mkIFxa yes no2)
