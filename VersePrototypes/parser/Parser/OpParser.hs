-- Vendored module to get the type right for Operator
-- |
...
addPrecLevel term ops =
  ...
  where
    ...
    ras' = pInfixR (\ x -> choice $ map ($ x) ras) term'
    las' = pInfixL (\ x -> choice $ map ($ x) las) term'
    nas' = pInfixN (\ x -> choice $ map ($ x) nas) term'
...
-- and the term.
pInfixN :: MonadPlus m => (a -> m (a -> a -> a)) -> m a -> a -> m a
pInfixN op p x = do
  f <- op x
  y <- p
  pure $ f x y
...
-- with parser @p@, then returns result of the operator application on @x@
-- and the term.
pInfixL :: MonadPlus m => (a -> m (a -> a -> a)) -> m a -> a -> m a
pInfixL op p x = do
  f <- op x
  y <- p
  let r = f x y
...
-- @x@ and the term.
pInfixR :: MonadPlus m => (a -> m (a -> a -> a)) -> m a -> a -> m a
pInfixR op p x = do
  f <- op x
