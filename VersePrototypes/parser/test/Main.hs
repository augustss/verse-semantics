...
import Parser.Verse qualified as P2
...
parser_data_dir :: String
parser_data_dir = "parser/test_data"

-- set verbose to true to see output of the parser
-- TODO: Jeff: use tasty
-- TODO: Jeff: RoundTrip testing
-- TODO: Jeff: Cleanup, specifcally remove desugarer and rewriter
main :: IO ()
main =
  do
    args <- getArgs
    ...
    case paths of
      [] -> do
        -- Jeff: this code reads in from stdin, I've disabled it so that we by
        -- default read from files
        -- contents <- ByteString.getContents
        -- getWorker options options "stdin" contents
        test_files <- listDirectory' parser_data_dir
        print test_files
        mapM_ (processFile options) test_files
