...
--
-- Test suite runner for verse-parser. This module currently includes parser
-- tests themselves. As it grows we'll split them out into separate modules.
--
-----------------------------------------------------------------------------
...
make_test p (desc, result) = testCase cleaned_desc $ verse_parser p desc @=? result
  where
...
roundtrip p desc = pretty_test p (desc,desc)
...
unitTests = testGroup "Unit Tests"
  [ syntax_dot_verse ] -- see parser/test_data/syntax.verse

-----------------------------------------------
--
--  syntax.verse: Each of these tests come from
--  the file $ROOT/VersePrototypes/parser/test_data/syntax.verse
--
-----------------------------------------------

syntax_dot_verse :: TestTree
syntax_dot_verse = testGroup "parser/test_data/syntax.verse"
  [ fixnums
  , chars
  , strings
  , identifiers
...
paths :: TestTree
paths =
  let passes = make_test pPath
  in testGroup "paths" $
     [ broken "something here" $ passes ("(monkey:)apa3", IdentName "I'm broken")
