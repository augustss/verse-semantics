...
pTestRunner' :: V.Parser TestRunner
pTestRunner' = V.choice
    [ V.pKeyword "tim" *> pure Tim_DS
...
pParserSkip' :: V.Parser Bool
pParserSkip' = do
  i <- V.pIdent
  guard (unLoc (fmap T.toLower i) == "libparser")
  _  <- V.pEqual
  sk <- V.pIdent
  case T.unpack $ unLoc sk of
    's':'k':'i':'p':_s -> pure  True
