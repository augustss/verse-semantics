module Epic.ShowRational (showRational) where
...
showRational Nothing rat =
  let (i, fr) = properFraction (abs rat) :: (Integer, Rational)
      si = if rat < 0 then "-" ++ show i else show i
      decimals ff = loop ff [] ""
      loop x fs ds =
        if x == 0
          then ds
          else case elemIndex x fs of
            Just j -> let (l, r) = splitAt j ds in l ++ "(" ++ r ++ ")"
            Nothing -> let (c, f) = properFraction (10 * x) :: (Integer, Rational) in loop f (fs ++ [x]) (ds ++ show c)
   in if fr == 0 then si else si ++ "." ++ decimals fr
