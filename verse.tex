...
It is easy to write an expression that yields too many values;
for example \vtt{def x in x} returns a sequence of all possible values.

Verse allows you to omit the \vtt{def} entirely; you may write, for example,
\vtt{(x + (x=7; 5)}.
There are simple, static rules that explain exactly where to place any missing \vtt{def}s,
as we describe in \Cref{sec:scope}.

In pracical Verse code, a programmer will typically write an equality before any uses;
...
Verse is designed to be statically type-checked; but the ``type system'' is actually
just more Verse code, so the type checker is necessarily partial.

As in any typed language, a type describes as set of values.  In
Verse, a ``type'' is just a particular sort of Verse expression.
...
\emph{characteristic function} of the set of values; a denotational semantics person might call it a \emph{retraction}. So:
\begin{itemize}
...
\item \vtt{false} is the function that always fails.
...
Verse has a unary prefix operator ``\vtt{:}'', where \vtt{:e} yields the sequence of all
values that inhabit the type \vtt{e}. But this is mere syntactic sugar:
\begin{center}
\begin{tabular}{rcl}
\vtt{:$e4} & $\equiv$ & \vtt{$e$(def x in x)}
\end{tabular}
\end{center}
As mentioned in \Cref{sec:any}, the expression \vtt{(def x in x)} enumerates all values; then
we apply the type $e$ (which is just a Verse function) to filter out just those values that belong to the type that $e$ describes.
