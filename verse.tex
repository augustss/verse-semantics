...
\begin{tabular}{|l|l|}
  ...
  Primitive & Comment \\
  \hline
  \vtt{def\{ $x_1$,$\cdots$,$x_n$ \}} & define variables $x_1$ etc. \\
  $x$ & use variable \\
  $i$ & literal \\
  ...
  \vtt{$e_1$,$\cdots$,$e_n$} & tuple formation \\
  \vtt{\{ $e_1$;$\cdots$;$e_n$ \}} & sequencing \\
...
\begin{tabular}{|l|l|l|}
  ...
  \vtt{$x$ :$e$} & \vtt{$x$ := :$e$} & \\
  \vtt{:}$e$ & \vtt{do \{ def\{x\}; $e$[x] \}} & $x$ fresh \\
  \vtt{$e_1$($e_2$)} & \vtt{if (x:=$e_1$[$e_2$]) then x else wrong} & $x$ fresh \\
  \vtt{typedef\{$e$\}} & \vtt{x => x = $e$} & $x$ fresh \\
  \vtt{$f$($e_1$) := $e_2$} & \vtt{$f$ := x => do \{ x = $e_1$; $e_2$ \}} \\
  \hline
\end{tabular}
