...
Verse is primarily a dynamically-typed functional language: the main syntactic construct is the \emph{expression}.
...
\item The expression \vtt{fail} yields no values at all, the empty sequence $[]$.  \simon{It's not \vtt{fail} is it? What is it?}
\end{itemize}
...
The comparison operators such as $(>)$, $(<)$, $(=)$ and so on, use the same idiom: \vtt{($e_1$ < $e_2$)} returns $e_2$ if the condition
...
This only works because \vtt{(<)} is left associative, so the expression means \vtt{(0<x) < 10}. (Exercise for the reader:
...
otherwise return nothing.   So \vtt{(1 | 8) < (2 | 10)} yeilds the sequence $[2,10,10]$.

\subsection{Variables}

In functional programming, a variable is bound to a specified immutable value as soon as it comes into scope.
Not so in Verse.  Instead, a variable is brought into scope \emph{without a specified value}.  Here is an example:
\begin{verbatim}
  def x in x + (x = 7; 5)
\end{verbatim}
