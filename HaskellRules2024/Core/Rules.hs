...
runtime = application
      <|> unification
      <|> normalization
      <|> substitution  -- SUBST, REC, EXI-APP
      <|> failAndErr
      <|> iter
      <|> dotDot        -- remove DOTDOT-EXPAND for verification!
...
appPrimOp =
  do label "APP"
     ...
     choices
       ...
       -- isGround (not used right now)
       , do label "-ISGROUND"
            Op IsGround :@: a <- lhs
            env <- error "get the environment"
...
matchExi_alphaRename zs e = choices (map pure (go zs e))
 where
  go zs e =
    [ cxe
    | Exi bnd <- [e]
