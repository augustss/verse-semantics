...
runTests test_flags
  ...
  , let fn = "<command-line>"
  = runTestFile tflg (fn, parseDie pTestFile fn s)
...
data Test
  ...
  | TestCoreEq TestInfo SrcCore SrcCore
...
srcToCore flags e = do { e1 :: Src.CoreExpr <- FrontEnd.Desugar.desugar e
                       ; e2 :: Rules.Expr   <- FrontEnd.ToCore.convertToCore flags e1
                       ; let e3 = Rules.Core.prep e2
                       ; return e2 }

evalExpr :: Rules.Expr -> Rules.Expr
evalExpr = TRS.Traced.term (Rules.normalize (Rules.everywhere TRS2024.rules))
...
assertEquivE ti flg e1 e2
  = do { c1 <- srcToCore flags e1
       ...
       ; assertEquiv ti flg (e1, c1) (e2, c2) a}
  where
...
assertEquiv :: (HasCallStack, Pretty a) => TestInfo -> TestFlags -> (a, Rules.Expr) -> (a, Rules.Expr) -> IO TestRes
