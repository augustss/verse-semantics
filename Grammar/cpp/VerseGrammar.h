...
enum class mode {None,Of,At,With};
...
struct text {
	...
	constexpr text(const char8* Start0,const char8* Stop0): Start(Start0), Stop(Stop0) {}
	constexpr text(const char8* Start0): text(Start0,Start0) {while(*Stop) ++Stop;}
	#if defined(__cpp_char8_t)
		constexpr text(const char* Start0) : text((const char8*)Start0, (const char8*)Start0) { while (*Stop) ++Stop; }
	#endif
...
constexpr token_info Tokens[]={
	...
	{u8"at"      , prec::Base  , mode::None   ,prec::Call   ,prec::Call   ,assoc::None      ,mode::At  , AllTokens },
	...
	{u8"of"      , prec::Base  , mode::None   ,prec::Call   ,prec::Call   ,assoc::None      ,mode::Of  , AllTokens },
...
//	{u8"&&"      , prec::Def   , mode::None   ,prec::And    ,prec::And    ,assoc::InfixRight,mode::With, AllTokens },
	{u8"'"       , prec::Base  , mode::None   ,prec::Never  ,prec::Never  ,assoc::None      ,mode::None, AllTokens },
	{u8"("       , prec::Base  , mode::None   ,prec::Call   ,prec::Call   ,assoc::None      ,mode::None, AllTokens },
	{u8")"       , prec::Never , mode::None   ,prec::Never  ,prec::Never  ,assoc::None      ,mode::None, AllTokens },
	{u8"*"       , prec::Prefix, mode::At     ,prec::Mul    ,prec::Mul    ,assoc::InfixLeft ,mode::At  , AllTokens },
	{u8"*="      , prec::Never , mode::None   ,prec::Def    ,prec::Def    ,assoc::InfixRight,mode::With, AllTokens },
	{u8"+"       , prec::Prefix, mode::At     ,prec::Add    ,prec::Add    ,assoc::InfixLeft ,mode::At  , AllTokens },
	{u8"+="      , prec::Never , mode::None   ,prec::Def    ,prec::Def    ,assoc::InfixRight,mode::With, AllTokens },
	{u8"-"       , prec::Prefix, mode::At     ,prec::Add    ,prec::Add    ,assoc::InfixLeft ,mode::At  , AllTokens },
	{u8"-="      , prec::Never , mode::None   ,prec::Def    ,prec::Def    ,assoc::InfixRight,mode::With, AllTokens },
	...
	{u8".."      , prec::Def   , mode::With   ,prec::To     ,prec::To     ,assoc::InfixRight,mode::At  , AllTokens },
	{u8"/"       , prec::Base  , mode::None   ,prec::Mul    ,prec::Mul    ,assoc::InfixLeft ,mode::At  , AllTokens },
	{u8"/="      , prec::Never , mode::None   ,prec::Def    ,prec::Def    ,assoc::InfixRight,mode::With, AllTokens },
	{u8":"       , prec::Def   , mode::With   ,prec::Call   ,prec::Choose ,assoc::None      ,mode::None, AllTokens },
	{u8":="      , prec::Never , mode::None   ,prec::Def    ,prec::Def    ,assoc::InfixRight,mode::With, AllTokens },
	{u8":)"      , prec::Never , mode::None   ,prec::Never  ,prec::Never  ,assoc::None      ,mode::None, AllTokens },
	{u8":>"      , prec::Never , mode::None   ,prec::Never  ,prec::Never  ,assoc::None      ,mode::None, AllTokens },
	{u8"<"       , prec::Base  , mode::None   ,prec::Call   ,prec::Less   ,assoc::InfixRight,mode::At  , AllowLess },
	{u8"<="      , prec::Never , mode::None   ,prec::Less   ,prec::Less   ,assoc::InfixRight,mode::At  , AllowLess },
	...
	{u8"=="      , prec::Never , mode::None   ,prec::Eq     ,prec::Never  ,assoc::None      ,mode::At  , AllTokens },
	{u8"=>"      , prec::Never , mode::None   ,prec::Fun    ,prec::Fun    ,assoc::InfixRight,mode::With, AllTokens },
	{u8">"       , prec::Never , mode::None   ,prec::Greater,prec::Greater,assoc::InfixRight,mode::At  , AllTokens },
	{u8">="      , prec::Never , mode::None   ,prec::Greater,prec::Greater,assoc::InfixRight,mode::At  , AllTokens },
	{u8"?"       , prec::Prefix, mode::At     ,prec::Call   ,prec::Call   ,assoc::Postfix   ,mode::With, AllTokens },
	{u8"@"       , prec::Expr  , mode::None   ,prec::Expr   ,prec::Expr   ,assoc::None      ,mode::None, AllTokens },
	{u8"["       , prec::Prefix, mode::At     ,prec::Call   ,prec::Prefix ,assoc::InfixRight,mode::At  , AllTokens },
	{u8"]"       , prec::Never , mode::None   ,prec::Never  ,prec::Never  ,assoc::None      ,mode::None, AllTokens },
	{u8"^"       , prec::Prefix, mode::At     ,prec::Call   ,prec::Call   ,assoc::Postfix   ,mode::With, AllTokens },
...
//	{u8"||"      , prec::Never , mode::None   ,prec::Or     ,prec::Or     ,assoc::InfixRight,mode::With, AllTokens },
	{u8"}"       , prec::Never , mode::None   ,prec::Never  ,prec::Never  ,assoc::None      ,mode::None, AllTokens },
};
...
inline const token_set DefinePostfixes    = token_set{u8"=",u8":=",u8"+=",u8"-=",u8"*=",u8"/="};
...
private:
	...
	struct expr {
		...
		block_t*                 InSpecifiers;
		expr(prec FinishPrec0,const cursor& Start0,expr* OuterExpr0,token_set AllowPostfixes0=token_set{},expr* QualIdentTarget0=nullptr,block_t* InSpecifiers0=nullptr):
			...
			QualIdentTarget(QualIdentTarget0), InSpecifiers(InSpecifiers0) {}
	...
	auto S15() {return Gen.Err(Snip(),"S15","Unexpected ",CursorQuote(),CursorText(),CursorQuote()," following number. If trying to call an extension function wrap number in parentheses `(num).ExtFunc()`.");}
	...
	// Should never happen.
	auto S100()          {return Gen.Err(Snip(),"S100","Unexpected error");}
	...
	result_t<syntax_t> Num() {
		...
		// Num   := !(("0b"|"0o"|"0u"|"0x") Hex) Digits ['.' Digits] Exp Units !('.' Digit)
		// Num   := "0x" Hex {Hex}  !('.' Alnum)
		auto Start=Cursor;
	...
	struct invoke: expr {
		...
		result_t<nothing> OnFinish(parser& Parser) override {
			...
			if(Clauses[0]) {
				...
				// See if these specifiers should land on grammar element Call or In.
				// Lands on In if present and followed by operator looser than prec::NotEq.
				expr* InExpr=nullptr;
				if(!this->ExprStop)
					for(InExpr=this; InExpr && !InExpr->InSpecifiers; InExpr=InExpr->OuterExpr) {}
				if(InExpr) {
					auto Saved = Parser.Cursor;
					if(Parser.Cursor.Pos==this->Finished->Pos && Parser.Cursor.Token==token(u8":")) {
						GRAMMAR_RUN(Parser.Space());
						if(Parser.Ending())
							InExpr=nullptr;
					}
					else if(Parser.Cursor.Pos==this->Finished->Pos && Parser.Cursor.Token==token::NewLine()) {
						capture_t Ignore;
						GRAMMAR_LET(BraceToken,Parser.ScanKey(Ignore,BracePostfixes));
						if(BraceToken!=token::None())
							InExpr=nullptr;
					}
					else if(Parser.Cursor.Token->PostfixPrec>=prec::NotEq)
						InExpr=nullptr;
					Parser.Cursor = Saved;
				}

				// Insert the calls and specifiers at prec::Call.
				...
				bool HandleInSpecifiers = InExpr && !Of;
				for(auto Call=FirstCall; Call; Call=Call->OuterCall) {
					...
					if(Call->CallMode==mode::With && HandleInSpecifiers) {
						GRAMMAR_LET(Specifier,Parser.Gen.Parenthesis(Call->CallParameter));
						if(Call->OuterCall && Call->OuterCall->CallMode==mode::With)
							Specifier=Parser.Gen.Trailing(Specifier,Call->CallParameter.BlockTrailing.TrailingCapture);
						Parser.Gen.SyntaxesAppend(InExpr->InSpecifiers->Elements,Specifier);
						InExpr->InSpecifiers->BlockTrailing=Call->CallParameter.BlockTrailing;
					}
					else {
						if(Call->CallMode!=mode::With)
							HandleInSpecifiers = InExpr != nullptr;
	...
	result_t<nothing> Invoke(invoke& Target,cursor BlockStart,capture_t TokenLeading=capture_t()) {
		...
		case token(u8"("): {
			...
			Block0.BlockSnippet      = Snip(BlockStart);
			auto NewCall             = call{u8"(",Cursor,mode::Of,Block0};
		...
		case token(u8","): case token(u8";"): case token(u8">"): case token(u8":>"): {
			...
			if(!Target.OuterExpr->MarkupTag)
				return S40();
			if(Target.PriorClause)
				ApplyTrailing(*Target.PriorClause,BlockStart);
	...
	result_t<nothing> In(expr& PostfixExpr,cursor Start,const ins* Ins=nullptr) {
		...
		if(InPrefixes.Has(Cursor.Token)) {
			...
			return In(PostfixExpr,Cursor,&NextIn);
		}
	...
	result_t<nothing> Define(expr& Target) {
		// Def := (.. | .. Space (('='|':='|'+='|'*='|'/=') Space (BraceInd | Def) | !'=' !':=')) {&In Def | ..}
		auto DefineToken=Cursor.Token;
		if(DefinePostfixes.Has(DefineToken)) {
			EatToken();
		...
		if(Prec<=BaseToken->PrefixPrec) {
			...
			case token(u8":"): case token(u8"in"): {
				...
				auto PostfixExpr = when_expr(prec::Def,&Target,AllTokens,Cursor,capture_t(),[&](expr& PostfixExpr)->result_t<nothing> {
					...
					if(Gen.SyntaxesLength(PostfixExpr.InSpecifiers->Elements)>0)
						GRAMMAR_RUN(UpdateFrom(Target,PostfixExpr.InSpecifiers->BlockTrailing,Gen.InSpecifiers(
							Snip(PostfixExpr.Start,*PostfixExpr.InSpecifiers->BlockTrailing.TrailingStart),
							ApplyTrailing(Target),PostfixExpr.InSpecifiers->Elements)));
					return Define(Target);
				},nullptr,&InSpecifiers);

				// In parses Choose into PostfixExpr, then Postfix extends to NotEq, then we handle InSpecifiers.
				GRAMMAR_RUN(In(PostfixExpr,Target.Start));
			...
			case token(u8"var"): case token(u8"set"): case token(u8"ref"): case token(u8"alias"): {
				...
				return WhenExpr(BaseToken->Symbol,prec::Choose,prec::Choose,&Target,Middle,[&](expr& Choose)->result_t<nothing> {
					...
					if(DefinePostfixes.Has(Cursor.Token)) // Translate "set x=3" to "set{x}:=3".
						return Define(Target);
					return nothing{};
				});
			}
			...
			case token(u8"&"): /*case token(u8"&&"):*/ {
				// Def := .. | ('&' | ..) Space Def
				EatToken();// Use Next(1) instead of EatToken if '&&' is a token.
				GRAMMAR_LET(Middle,Space());
		...
		token_leading_loop:
			...
			case token(u8">"):
				if(Target.MarkupStart)
					goto markup_postfix;
				[[fallthrough]]; // Else continue on as greater operator.
			case token(u8"*"):  case token(u8"/"): case token(u8"&"):
			case token(u8"+"):  case token(u8"-"):  
	...
	template<class f> struct when_expr: expr {
		...
		when_expr(prec FinishPrec0,expr* OuterExpr0,token_set PostfixAllow0,const cursor& Start0,
			const capture_t& ExprLeading0,const f& F0,expr* QualIdentTarget0=nullptr,block_t* InSpecifiers0=nullptr):
			expr(FinishPrec0,Start0,OuterExpr0,PostfixAllow0,QualIdentTarget0,InSpecifiers0), F(F0) {
...
template<class gen_t> struct generate: gen_t {
	...
	result<syntax_t,error_t> Units(const snippet& Snippet,const syntax_t& Num,text Units) const {
		...
		return this->Call(Snippet,mode::Of,UnitsIdent,block_t{Snippet,Parameters});
	}
	...
	result_t<syntax_t> StringInterpolate(const snippet& Snippet,place Place,bool /*Brace*/,const block_t& Block) const {
		...
		return this->Call(Snippet,mode::Of,FunctionSyntax,Block);
	}
	result_t<syntax_t> String(const snippet& Snippet,const syntaxes_t& Splices) const {
		...
		return this->Call(Snippet,mode::Of,FunctionSyntax,block_t{snippet{},Splices,form::Commas});
	}
	...
	result_t<syntax_t> PrefixBrackets(const snippet& Snippet,const block_t& Left,const block_t& Right) const {
		...
		if(this->SyntaxesLength(Left.Elements)==0) {
			...
			return this->Call(Snippet,mode::At,Macro,Right);
		}
		...
		return this->Call(Snippet,mode::At,Macro,Parameters);
	}
	...
	result<syntax_t,error_t> InSpecifiers(const snippet& Snippet,const syntax_t& In,const syntaxes_t& InSpecifiers) const {
		block_t InBlock;
		this->SyntaxesAppend(InBlock.Elements,In);
		InBlock.Specifiers=InSpecifiers;
		GRAMMAR_LET(Macro,this->Ident(snippet{},u8"in_with",u8"",u8""));
		return this->Invoke(Snippet,Macro,InBlock,nullptr,nullptr);
	}
