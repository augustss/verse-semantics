...
isectM s Nothing = s
isectM s (Just u) | u `sIn` s = return u
                  | otherwise = empty
...
dM :: Exp -> Maybe W -> Env -> WS
dM (Var x) u rho = lookupEnv x rho `isectM` u
dM (Int k) u _rho = return (VInt k) `isectM` u
...
dM (Colon e) (Just u) rho = do f <- dE e rho; apply f u
